            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item"><a href="<c:url value="/banner.do"/>">배너관리</a></li>
	    ...
	    <div class="col-12">
		    <div class="card">
			    <div class="card-body">
				    ...
				    <form id="bannerSearchForm" name="bannerSearchForm" class="mb-3" action="/banner.do">
						<input type="hidden" name="pageIndex" value="${searchParams.pageIndex}"/>
	    
					    <div class="row mb-2">
					        <div class="col-sm-4 col-lg-6">
					            <a href="<c:url value="/banner/edit.do"/>" class="btn btn-info btn-sm btn-rounded mb-2">
					            	<i class="mdi mdi-plus"></i> 배너 등록
					            </a>
					        </div>
						...
						<div class="collapse show" id="searchCollapse">
							<div class="form-group row">
								<div class="col">
									...
									<input type="text" id="searchText" name="searchText" class="form-control" value="${searchParams.searchText}" placeholder="미디어명 / 배너명">
								</div>
							</div>
						</div>
						...
						<table id="order-datatable" class="table table-centered table-hover w-100 nowrap dataTable">
						    <thead class="table-light">
						        <tr>
						            ...
						            <th scope="col">사진(썸네일)</th>
						            <th scope="col">배너명</th>
						            <th scope="col">미디어명</th>
						            <th scope="col">노출시작</th>
						            <th scope="col">노출종료</th>
						            <th scope="col" class="text-center">비공개여부</th>
						    ...
						    <c:choose>
						    	<c:when test="${not empty list}">
							    	...
							    	<tr>
							    		<td>${item.num}</td>
							    		<td>${item.bannerId}</td>
							    		<td>${fn:replace(item.bannerPath, replaceSrc, replaceTrg)}/${item.bannerThumb}</td>
							    		<td>${item.title}</td>
							    		<td>${item.mediaId}</td>
							    		<td>${item.displayStart}</td>
							    		<td>${item.displayEnd}</td>
							    		<td class="text-center">${item.hide}</td>
							    		<td class="text-center">
							    			<a class="action-icon text-info" href="/banner/edit.do?banner_id=${item.banner_id}" title="수정하기">
