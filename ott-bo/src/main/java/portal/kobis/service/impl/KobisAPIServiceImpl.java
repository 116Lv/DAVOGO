...
public class KobisAPIServiceImpl implements KobisAPIService {
	...
	@Autowired
	private KobisOpenAPIRestService kobisService;
	...
	public void saveDailyMovieList() {
		try {
			...
			String dailyResponse = kobisService.getDailyBoxOffice(true, DateUtil.getYesterday(), itemPerPage, "", "", "");
	...
	private Map<String, Object> getMovieInfo(String movieCd) {
		...
		try {
			...
			String movieInfoResponse = kobisService.getMovieInfo(true, movieCd);
		
			ObjectMapper mapper = new ObjectMapper();
			Map<String, Object> movieResult = mapper.readValue(movieInfoResponse, HashMap.class);
			for(Entry<String, Object> entry : movieResult.entrySet()) {
				log.debug("key = "+entry.getKey()+" / value = "+entry.getValue());
			}
	...
	public void saveWeeklyMovieList() {
		...
		try {
			...
			String weeklyResponse = kobisService.getWeeklyBoxOffice(true, DateUtil.getYesterday(), itemPerPage, "0", "", "", "");
